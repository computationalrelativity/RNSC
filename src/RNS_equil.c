		 ...
		 double *J)
{
 ...
 D_m        = malloc((SDIV+1)*sizeof(double));
 D_m_0      = malloc((SDIV+1)*sizeof(double));
 D_m_p      = malloc((SDIV+1)*sizeof(double));
 D_J        = malloc((SDIV+1)*sizeof(double));
 d_o_e      = malloc((SDIV+1)*sizeof(double));
 d_g_e      = malloc((SDIV+1)*sizeof(double));
 d_r_e      = malloc((SDIV+1)*sizeof(double));
 d_v_e      = malloc((SDIV+1)*sizeof(double));
 gama_mu_1  = malloc((SDIV+1)*sizeof(double));
 gama_mu_0  = malloc((SDIV+1)*sizeof(double));
 rho_mu_0   = malloc((SDIV+1)*sizeof(double));
 rho_mu_1   = malloc((SDIV+1)*sizeof(double));
 omega_mu_0 = malloc((SDIV+1)*sizeof(double));
 D_T        = malloc((SDIV+1)*sizeof(double));
...
double   sum_rho=0.0,         /* intermediate sum in eqn for rho */
  ...
  *gama_mu_1,            /* gama at \mu=1 */
  *gama_mu_0,             /* gama at \mu=0 */
  *rho_mu_1,              /* rho at \mu=1 */
  *rho_mu_0,              /* rho at \mu=0 */
  *omega_mu_0,           /* omega at \mu=0 */
         s_e=0.5,
       **da_dm,
       **dgds,
       **dgdm,
 ...
 sin_theta = malloc((MDIV+1)*sizeof(double));
 theta = malloc((MDIV+1)*sizeof(double));
 d_e_ds_mu_0 = malloc((SDIV/2)*sizeof(double));
 ...
 omega_mu_0=malloc((SDIV+1)*sizeof(double));
    ...
    f2n = dmatrix(1,RNS_lmax+1,1,SDIV);
    f_rho = d3tensor(1,SDIV,1,RNS_lmax+1,1,SDIV);
    f_gama = d3tensor(1,SDIV,1,RNS_lmax+1,1,SDIV);
  ...
  n=0;
   for(i=1;i<=MDIV;i++)
      P_2n[i][n+1]=nr_legendre(2*n,mu[i]);

   for(i=1;i<=MDIV;i++)
     for(n=1;n<=RNS_lmax;n++) {
      P_2n[i][n+1]=nr_legendre(2*n,mu[i]);
      P1_2n_1[i][n+1] = nr_plgndr(2*n-1 ,1,mu[i]);
    }

  free_dmatrix(f2n,1,RNS_lmax+1,1,SDIV);
  ...
  while(dif> accuracy || n_of_it<2) { 
	...
	ERROR("rns: Max number of iteration reached ! Exiting. \n");
      }
	...
	/* differential rotation */
      ...
      for(s=1;s<=SDIV;s++) {
         ...
         for(m=1;m<=MDIV;m++) {
	      ...
	      /* 
		 =================================================
		 this is the original comment in Whisky_RNSID
		 =================================================
	      */
	      
	      /* ENABLE LATER FOR TAB EOS 
		 
	      if(strcmp(interp_type,"GDI")==0) {
	      
	      if(e_cl==0)  
	      *** NORMAL TAB EOS ***
	      interp_GD( log_h_tab, log_p_tab, dlogp_dlogh_tab, 
	      n_tab, enthalpy[s][m], &pressure[s][m],
	      &energy[s][m] );
	      else { 
	      *** STRANGE STAR WITH CRUST 
	      TEMP. FIX FOR STRANGE STAR WITH CRUST - ONLY WITH GDI ***
	      if(enthalpy[s][m]< h_cl) 
	      interp_GD( log_h_tab, log_p_tab, 
	      dlogp_dlogh_tab, n_tab, enthalpy[s][m], 
	      &pressure[s][m], &energy[s][m] );
	      else {
	      
	      pressure[s][m] = ( (e_cl+p_cl)*exp((1.0+e_coeff)
	      *(enthalpy[s][m]-h_cl))
	      -e_cl+p_cl*e_coeff)/(e_coeff+1.0);
	      energy[s][m] = e_cl+(pressure[s][m]-p_cl)*e_coeff;     
	      }   
	      }	
	      }else{   
	      ...
	      /* 
		 =================================================
		 interp_type,h_cl,p_cl,e_cl are NOT defined !!!
		 =================================================
	      */
	      
	      /* 
		 =================================================
		 this is taken from the free version (v2) of rns
      ...
      D1_rho = dmatrix(1,RNS_lmax+1,1,SDIV);
      D1_gama = dmatrix(1,RNS_lmax+1,1,SDIV);
      ...
      D2_rho = dmatrix(1,SDIV,1,RNS_lmax+1);
      D2_gama = dmatrix(1,SDIV,1,RNS_lmax+1);
      ...
      free_dmatrix(D1_rho,1,RNS_lmax+1,1,SDIV);
      free_dmatrix(D1_gama,1,RNS_lmax+1,1,SDIV);
      ...
      free_dmatrix(D2_rho,1,SDIV,1,RNS_lmax+1);
      free_dmatrix(D2_gama,1,SDIV,1,RNS_lmax+1);
...
/*      (*Omega) = Omega_h*C/(r_e*sqrt(KAPPA));/*this should be a conversion*/
    ...
    free_d3tensor(f_rho, 1,SDIV,1,RNS_lmax+1,1,SDIV);
    free_d3tensor(f_gama,1,SDIV,1,RNS_lmax+1,1,SDIV);
