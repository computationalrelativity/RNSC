...
./kepler.x -q tab -f ../eos/eosA -e 1e15 
	      ...
	      double W, double T_o_W)
{
  ...
  printf(
	 "%4.3f \t%4.1f \t%4.3f \t%4.3f \t%4.3f \t%4.1f \t%5.1f \t%4.2f \t%4.3f \t%4.3e \t%4.3f \n",
	 r_ratio,
	 e_center,
	 Mass/MSUN,
	 Mass_0/MSUN,
	       ...
	       double W, double T_o_W)
{
  ...
  printf(
	 "%4.3f \t%4.3f \t%4.3f \t%4.3f \t%4.3f \t%4.3f \t%5.3f \t%4.2f \t%4.3f \t%4.3e \t%4.3f \n",
	 r_ratio,
	 e_center,
	 Mass,
	 Mass_0,
         ...
         char **argv)                 /* Command line arguments */
{
 ...
 double ebeg=0, efin=0, stepec=0; /* initial/final value of e_central in sequence */
 int nmodels = 1; /* no models in sequence */
 ...
 float ans, fh,fl,fm,fnew,sroot,xh,xl,xm,xnew,xacc;
  ...
  for(i=1;i<argc;i++) 
	...
	/*   e_center *= C*C*KSCALE; */
	/* break; */
	sscanf(argv[i+1],"%lf",&ebeg);
	break;

      case 'l':
	sscanf(argv[i+1],"%lf",&efin);
	break;

      case 'n':
	sscanf(argv[i+1],"%d",&nmodels);
	break;
	...
	fprintf(stderr,"  -e central energy density in gr/cm^3 (model or begin val for sequence)\n");
	fprintf(stderr,"  -l central energy density in gr/cm^3 (last value for sequence)\n");
	fprintf(stderr,"  -n no models in sequence\n");
  ...
  /* CHECK e_central INPUT */
  if (ebeg<=0.) { 
    printf("Need to set central density >0.");
    exit(0);
  }
  if (nmodels<=1 || efin<=0. || efin<=ebeg) { 
    nmodels =1;
    efin = ebeg;
  }
  if (nmodels>1 && efin>ebeg) {
    //stepec = (efin-ebeg)/(nmodels-1);
    stepec = exp((log10(efin)-log10(ebeg))/(nmodels-1));    
  }
  ...
  printf("# ratio\te_15\tM\tM_0\tr_star\tspin\tOmega_K\tI\tJ/M^2\tW\tT/W\n");
  if(strcmp(eos_type,"tab")==0)
    printf("# \tg/cm^3\tsun\tsun\tkm\ts-1\ts-1\tg cm^2\tc=G=1\t???\t\n");
  ...
  /* FOR A SEQUENCE */
  int index=0;
  for (index=0; index<nmodels; index++) {
    e_center = ebeg + index*stepec;
    ...
    mass_radius( s_gp, mu, log_e_tab, log_p_tab, 
		 ...
		 &Mass, &Mass_0, &RotEne, &BinEne, &J, &R_e, v_plus, v_minus, &Omega_K);
    
    /* PRINT OUT INFORMATION ABOUT THE STELLAR MODEL */
    
    if (VERBOSE) {
      if(strcmp(eos_type,"tab")==0)
	printtab(r_ratio,e_center, Mass, Mass_0, R_e, Omega, Omega_K, J, BinEne,RotEne/BinEne);
      else 
	printpoly(r_ratio,e_center, Mass, Mass_0, R_e, Omega, Omega_K, J, BinEne,RotEne/BinEne);
    }
    
    dr=0.05;
    
    if (VERBOSE) printf("Look for the Kepler limit...\n");
